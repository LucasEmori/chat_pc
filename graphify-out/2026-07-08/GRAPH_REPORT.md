...
## Corpus Check
- 20 files · ~36,226 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 377 nodes · 617 edges · 50 communities (17 shown, 33 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 27 edges (avg confidence: 0.64)
...
- [[_COMMUNITY_tamanhos.py|tamanhos.py]]
...
- `_normalizar_pc()` --calls--> `expandir_lista()`  [EXTRACTED]
  _app_streamlit_legacy.py → src/tamanhos.py
- `_montar_estado_inicial()` --calls--> `estado_inicial()`  [EXTRACTED]
  _app_streamlit_legacy.py → src/graph.py
- `_render_msg()` --calls--> `linhas_para_dataframe_pc()`  [EXTRACTED]
  _app_streamlit_legacy.py → src/cleaner.py
...
Cohesion: 0.09
Nodes (34): Any, _cleanup_asyncio_tasks(), _config(), _df_preview(), _esc(), _file_chip_html(), _label_row(), _msg_assistant() (+26 more)
...
Cohesion: 0.09
Nodes (35): configure(), Series, _aplicar_conversao_cny(), _detectar_coluna_tamanho(), detectar_fator_cny(), _detectar_linha_header(), _e_footer(), limpar_planilha() (+27 more)
...
Cohesion: 0.06
Nodes (44): anel_decide(), AnelDecisionIn, ConfigureIn, create_session(), _df_preview_records(), hitl_resolve(), HitlResolveIn, _linhas_to_records() (+36 more)
...
### Community 20 - "tamanhos.py"
Cohesion: 0.27
Nodes (9): expandir_linhas(), expandir_lista(), extrair_tamanho_cm(), _parse_anel(), Regras de Tamanho do Pedido de Compra por categoria.  Roda DEPOIS do LLM e ANTES, Aplica a regra de tamanho a uma LinhaPedidoCompra (dict).      Retorna 1+ linhas, Aplica expandir_linhas a cada item e achata o resultado., Soma todas as medidas no texto e retorna o inteiro truncado.      "15,5CM+1CM+1C (+1 more)
...
- **Are the 6 inferred relationships involving `SessionState` (e.g. with `AnelDecisionIn` and `ConfigureIn`) actually correct?**
  _`SessionState` has 6 INFERRED edges - model-reasoned connections that need verification._
